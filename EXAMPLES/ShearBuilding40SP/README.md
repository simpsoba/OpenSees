...
| `ShearBuilding40Partition.tcl` | Full-mesh + `partition` (`OpenSeesMP`, custom or METIS) |
...
Full-mesh + `partition` (analytical, no OpenFresco; Linux/OpenSeesMP):

```bash
# even story bands via -customPartition
mpiexec -n 4 OpenSeesMP ShearBuilding40Partition.tcl
mpiexec -n 8 OpenSeesMP ShearBuilding40Partition.tcl
...
| `ShearBuilding40Partition.tcl` | `output-partition-custom/` or `output-partition-metis/` | (analytical only) |
