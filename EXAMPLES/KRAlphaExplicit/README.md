...
- `Two-Story_MRF/`: two-story MRF model from Kolay & Ricles papers + ground motion file.
  ...
  - OpenSeesMP: `two_story_MRF_mp.tcl` + `run_mp_integrators.sh` (METIS `partition`, MultiSOE×Mumps/DistributedCuDSS/optional ParallelProfileSPD, Cuda×DistributedCuDSS)
...
- OpenSeesMP backend rules for this sandbox:
  - `*MultiSOE*` → `Mumps` or `DistributedCuDSS` (primary); `ParallelProfileSPD` is an optional CPU SPD fallback
...
- Serial MultiSOE + ProfileSPD:
  - `OpenSees tests/multisoe_profile_smoke.tcl`
