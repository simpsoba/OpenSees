...
for {set i $flagStart} {$i < $argc} {incr i} {
    ...
    switch -exact $flagLower {
        ...
        -constraints {
            incr i
            if {$i >= $argc} {
                puts stderr "ERROR: -constraints requires Transformation or Penalty"
                exit 1
            }
            set constrHandler [lindex $argv $i]
            if {$constrHandler ni {Transformation Penalty}} {
                puts stderr "ERROR: -constraints must be Transformation or Penalty"
                exit 1
            }
        }
...
proc applyConstraints {} {
    global constrHandler penaltyAlpha
    if {$constrHandler eq "Penalty"} {
        constraints Penalty $penaltyAlpha $penaltyAlpha
    } else {
