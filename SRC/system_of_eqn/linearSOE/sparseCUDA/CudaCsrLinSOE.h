...
// $Source: OpenSees/SRC/system_of_eqn/linearSOE/sparseCUDA/CudaCsrLinSOE.h
...
#ifndef CudaCsrLinSOE_h
#define CudaCsrLinSOE_h
...
class CudaCsrLinSOE : public LinearSOE
{
    public:
        ...
        CudaCsrLinSOE(LinearSOESolver &theSolver, 
        ...
        int setCudaCsrLinSolver(LinearSOESolver &newSolver);   
        int solve(void);
        int saveSparseA(OPS_Stream& output, int baseIndex = 0);
        void copyFromHostToDevice(void);
        void copyFromDeviceToHost(void);

        int sendSelf(int commitTag, Channel &theChannel);   
        int recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker);  

        // Track changes in the matrix
        enum class CudaCsrMatrixStatus {
        ...
        template<typename DataType>
        friend class CudaCsrLinSolver;

    protected:

    private:    
        // Track the status of the matrix
        CudaCsrMatrixStatus m_matrixStatus;

        // Host representation
        Vector m_X, m_B; // for interfacing with OpenSees, wraps padded vectors
        std::vector<double> m_XPadded, m_BPadded; // for internal use
        ID m_ARowPtrBlock, m_AColIdxBlock;
        std::vector<double> m_AValuesBlock;
        
        // Device representation
        int *m_dARowPtrBlock, *m_dAColIdxBlock;
        DataType *m_dAValuesBlock, *m_dBptr, *m_dXptr;
        
        // Pinned host buffers
        CudaPinnedVector<int> m_PinnedIntBuffer;
        CudaPinnedVector<DataType> m_PinnedDataBuffer;
        
        // Device buffers
        int *m_dIntBuffer;
        DataType *m_dDataBuffer;
...
inline OPS_Stream& operator<<(OPS_Stream& os, CudaCsrLinSOE::CudaCsrMatrixStatus status) {
    switch (status) {
        case CudaCsrLinSOE::CudaCsrMatrixStatus::UNCHANGED:
            return os << "UNCHANGED";
        case CudaCsrLinSOE::CudaCsrMatrixStatus::COEFFICIENTS_CHANGED:
            return os << "COEFFICIENTS_CHANGED";
        case CudaCsrLinSOE::CudaCsrMatrixStatus::STRUCTURE_CHANGED:
